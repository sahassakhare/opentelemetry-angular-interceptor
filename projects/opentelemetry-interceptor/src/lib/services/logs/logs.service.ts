...
export class OpenTelemetryLogsService {
  ...
  private emitLogRecord(level: string, message: string, attributes: Record<string, any> = {}): void {
    ...
    try {
      ...
      if (this.config.console) {
        ...
        console[consoleMethod](`[OTEL-LOG:${level.toUpperCase()}]`, message, finalAttributes);
      }
    } catch (error) {
  ...
  private initializeConsoleBridge(): void {
    ...
    console.log = (...args: any[]) => {
      this.info(args.join(' '), { source: 'console.log' });
      originalMethods.log.apply(console, args);
    };

    console.info = (...args: any[]) => {
      this.info(args.join(' '), { source: 'console.info' });
      originalMethods.info.apply(console, args);
    };

    console.warn = (...args: any[]) => {
      this.warn(args.join(' '), { source: 'console.warn' });
      originalMethods.warn.apply(console, args);
    };

    console.error = (...args: any[]) => {
      this.error(args.join(' '), { source: 'console.error' });
      originalMethods.error.apply(console, args);
    };

    console.debug = (...args: any[]) => {
      this.debug(args.join(' '), { source: 'console.debug' });
